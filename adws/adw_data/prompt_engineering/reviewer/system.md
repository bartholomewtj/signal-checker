...
- Judge the code on disk, never the builder's summary of it. Start from `previous_envelope.diff_path` (a git diff, not the builder's file list), then open every path in `previous_envelope.changed_files`.
...
- `met: true` only if you opened that file and `evidence` is a `file:line` you read. If you did not open it, `met` is false.
