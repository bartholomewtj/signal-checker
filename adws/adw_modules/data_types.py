...
class AgentConfig(BaseModel):
    name: str
    coding_agent: Literal["pi", "claude_code", "agy", "gemini"] = "claude_code"
    model: str = "sonnet"
...
#
# `PiRequest` / `PiResult` keep their upstream names, but they are the SHARED
# contract every coding agent speaks — `agent_cc.py` fills the same objects as
# `agent_pi.py`. Read "Pi" here as "the coding agent", not as the pi binary.
# Renaming them would touch every call site and gate for no behavioural gain.
...
class PiRequest(BaseModel):
    ...
    thinking: str = "medium"
    session_id: str                 # same id = same context window, both interfaces
    session_dir: str
...
class UsageBreakdown(BaseModel):
    ...
    `agent_cc.py` translates Claude Code's usage object into this same shape at
    its boundary, so one vocabulary reaches the trace regardless of who ran.

    On Claude Code the per-component COST fields stay zero: the CLI prices a
    whole run on its result event rather than per turn, so only `total_cost` is
    populated — and it is the notional API price, not money billed.
    """
    input_tokens: int = 0
    output_tokens: int = 0
